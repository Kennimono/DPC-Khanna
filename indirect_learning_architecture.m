function [h, err]=indirect_learning_architecture(x, z, y, h, N_use, P, M)
% warning('off','all')
...
% x      original tx samples (not used)                        size N x 1
...
for n=1:N_use
    for p=1:P
        ...
        for m=floor(M/2):-1:-floor(M/2)
            z_hat(n)=z_hat(n)+h(l)*y(n+floor(M/2)-m).^p;
            l=l+1;
        end
    end
end
...
% h_hat=inv(B.'*B)*B.'*z_hat; % be careful to do matrix multiplication with B, not elementwise!
