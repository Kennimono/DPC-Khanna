function [h, h_inv, err]=ila_nonlin_inv(x, z, y, h, N_use, P, M)
% warning('off','all')
...
for p=1:P
    ...
    for m=floor(M/2):-1:-floor(M/2)
        %         B_m(:,m)=circshift(y.'.^p,m-1);
        B_m_inv(:,l)=[x(floor(M/2)-m+1:N_use+floor(M/2)-m)].^p;
        l=l+1;
    end
    B_inv=[B_inv B_m];
end
...
h_inv=inv(B_inv.'*B_inv)*B_inv.'*y(1+floor(M/2):end-floor(M/2));
